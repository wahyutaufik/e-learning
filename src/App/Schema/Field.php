...
abstract class Field implements \ArrayAccess, \Iterator, \JsonKit\JsonSerializer
{
    ...
    public function label($plain = false)
    {
        ...
        return '<label class="col-sm-3 control-label no-padding-right">'.$label.($this['filterRequired'] ? '*' : '').'</label>';
    }
    ...
    public function formatReadonly($value, $entry = null)
    {
        return '
            <div class="col-sm-9">
                <input type="text" value="'.($this->formatPlain($value, $entry) ?: '&nbsp;').'" readonly />
            </div>
        ';
    }
    ...
    public function formatInput($value, $entry = null)
    {
        ...
        return '
			<div class="col-sm-9">
        		<input type="text" name="'.$this['name'].'" value="'.$value.'" placeholder="'.l($this['label']).
            	'" autocomplete="off" />
            </div>
        ';
    }
